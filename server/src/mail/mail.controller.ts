...
export async function userVerificationMail(user: UserModel, origin: string, verificationToken: string): Promise<boolean | Error> {

  const formMail = userVerificationTemplate(user, origin, verificationToken);

  return transporter.sendMail(formMail).then(() => true)
    .catch(error => error as Error);
}

export async function passwordRecoveryMail(user: UserModel, origin: string, verificationToken: string): Promise<boolean | Error> {
