...
import { ErrorMessage, ManyTransactionModel, ReqQuery, RequestId, UserModel } from '../types/types';
...
// Login check
export function loginCheck(): string {
  return loginSuccess;
}
...
export async function userResetPasswordMany(req: RequestId): Promise<string> {
  ...
  const result = await Promise.all(transactions.map(async id => {
    ...
    if (error) return { id, error: error.response };

    return { id, success: 'Mail send successful' };
  }));

  const isError = result.some(x => x.error);
